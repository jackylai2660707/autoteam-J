  ...
  <div>
    <div class="mb-6">
      ...
      <p class="mt-2 max-w-4xl text-sm leading-6 text-gray-400">
        只做 CPA quota 检查、seat 收敛和 CPA OAuth active/disabled 启停。配置多个 Team 后会逐个 Team 独立调度；某个 Team 全员 quota 耗尽时才消费该 Team 的 pending invite，全程不 kick/remove/cancel。
      </p>
    </div>
    ...
    <div class="mb-6 grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
        ...
        <div v-else-if="teams.length" class="grid gap-3 md:grid-cols-2">
          ...
          >
            ...
            <div class="mt-3 flex flex-wrap gap-2 text-[11px] text-gray-400">
              <span class="rounded bg-gray-800 px-2 py-1">active {{ team.max_chatgpt_active || 2 }}</span>
            ...
            <div class="mt-2 flex flex-wrap gap-2 text-[11px] text-gray-400">
              ...
              <span class="rounded bg-emerald-500/10 px-2 py-1 text-emerald-300">
                quota 可用 {{ quotaSummaryForTeam(team).available }}
              </span>
      ...
      <div class="rounded-2xl border border-blue-500/20 bg-blue-500/10 p-4 text-sm leading-6 text-blue-200">
        <div class="font-medium text-blue-100">调度策略</div>
        <ol class="mt-2 list-decimal space-y-1 pl-5 text-xs text-blue-200/90">
          <li>先从 CPA 检查该 Team 内成员 5h + weekly quota。</li>
          <li>只保留 1~5 个 quota 可用账号为 ChatGPT/OAuth active，其余 Codex + disabled standby。</li>
          <li>无可用 quota 时不做无用 swap；开启自动替换后才消费该 Team pending invite。</li>
          <li>安全闸禁止 kick/remove/cancel invite；手动按钮也只提交 swap 或 pending invite 消费任务。</li>
        </ol>
      </div>
    </div>
    ...
    <div class="mb-6 grid gap-4 lg:grid-cols-[0.95fr_1.05fr]">
      ...
      <div class="rounded-2xl border border-white/10 bg-gray-900/70 p-4">
        <div class="mb-3 flex items-center justify-between gap-3">
          <div>
            <h3 class="text-sm font-semibold text-white">quota 缓存 / 刷新时间</h3>
            <p class="mt-1 text-xs text-gray-500">只读展示本地缓存；新账号没有记录时会在 swap_seat 中先跑一次 quota。</p>
          </div>
        ...
        <div v-if="runtimeLoading" class="h-20 animate-pulse rounded-xl bg-gray-800/70"></div>
        <div v-else-if="quotaEntries.length" class="overflow-x-auto">
          <table class="w-full text-xs">
            <thead>
              <tr class="border-b border-gray-800 text-left text-gray-500">
                <th class="px-2 py-2 font-medium">邮箱</th>
                <th class="px-2 py-2 font-medium">Team</th>
                <th class="px-2 py-2 font-medium">5h / weekly</th>
                <th class="px-2 py-2 font-medium">状态</th>
                <th class="px-2 py-2 font-medium">下次检查</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="entry in quotaEntries" :key="entry.key" class="border-b border-gray-800/50">
                <td class="px-2 py-2 font-mono text-slate-300">{{ entry.email || '-' }}</td>
                <td class="px-2 py-2 font-mono text-gray-500">{{ entry.account_id || 'default' }}</td>
                <td class="px-2 py-2 text-gray-300">{{ entry.primary_remaining }}% / {{ entry.weekly_remaining }}%</td>
                <td class="px-2 py-2">
                  <span class="rounded-full px-2 py-0.5" :class="quotaStateClass(entry)">
                    {{ quotaStateLabel(entry) }}
                  </span>
                </td>
                <td class="px-2 py-2 text-gray-500">{{ formatTs(entry.next_check_at || entry.exhausted_until) }}</td>
              </tr>
            </tbody>
          </table>
        </div>
        <div v-else class="rounded-xl border border-dashed border-gray-700 px-4 py-6 text-sm text-gray-500">
          暂无 quota 缓存。第一次执行 swap_seat 后会记录每个 OAuth 的 5h / weekly 剩余额度和 reset 时间。
        </div>
      </div>
    </div>
    <TaskPanel
      ...
      <div class="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p class="mt-1 text-xs leading-5 text-gray-500">
            这里直接读取 CPA auth-files，只允许手动 disable；enable 必须由 swap_seat 根据 quota 和保留数自动选择。
          </p>
        </div>
        <div class="flex flex-wrap items-center gap-2">
          <span class="rounded bg-emerald-500/10 px-2 py-1 text-xs text-emerald-300">active {{ cpaSummary.active }}</span>
        ...
        <div v-for="auth in codexAuths" :key="auth.name || auth.id || auth.auth_index" class="rounded-xl border border-white/10 bg-white/[0.03] p-3">
          ...
          <div class="mt-3 flex items-center justify-between gap-3">
            ...
            <button
              ...
              :disabled="!!runningTask || cpaActionName === (auth.name || auth.id)"
            ...
            >
              {{ cpaActionName === (auth.name || auth.id) ? '处理中...' : 'disable' }}
            </button>
          </div>
        </div>
      </div>
      <div v-else class="rounded-xl border border-dashed border-gray-700 px-4 py-6 text-sm text-gray-500">
        暂未读取到 CPA Codex OAuth。请先在「配置面板 → CPA 管理」配置 CPA_URL / CPA_KEY。
      </div>
    </div>
  </div>
</template>
...
const quotaEntries = computed(() => allQuotaEntries.value.slice(0, 8))
...
function quotaSummaryForTeam(team) {
  const accountId = String(team?.account_id || '').trim()
  const entries = allQuotaEntries.value.filter(entry => String(entry.account_id || '').trim() === accountId)
  return {
...
async function disableCpaAuth(auth) {
  const name = String(auth?.name || auth?.id || '').trim()
